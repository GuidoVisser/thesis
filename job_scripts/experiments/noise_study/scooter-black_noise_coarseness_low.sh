...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --noise_upsample_size 64 \
            --noise_temporal_coarseness 8

echo "$SLURM_JOBID | End:   $(date)" >> $HOME/thesis/job_logs/run_layer_decomposition.log

#Copy output directory from scratch to home
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_noise_study__noise_coarseness_low
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_noise_study__noise_coarseness_low

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO noise_study__noise_coarseness_low $t >> $HOME/thesis/times.txt