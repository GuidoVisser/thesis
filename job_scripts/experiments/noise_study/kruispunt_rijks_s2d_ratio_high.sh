...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'The input noise is only static' \
            --num_static_channels 13

echo "$SLURM_JOBID | End:   $(date)" >> $HOME/thesis/job_logs/run_layer_decomposition.log

#Copy output directory from scratch to home
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_noise_study__s2d_ratio_high
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_noise_study__s2d_ratio_high
