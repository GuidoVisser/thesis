...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'A 3D bottleneck network where the 3D convolution is a normal 3D convolution, not transposed' \
            --model_setup 3

echo "$SLURM_JOBID | End:   $(date)" >> $HOME/thesis/job_logs/run_layer_decomposition.log

#Copy output directory from scratch to home
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_3d_conv_study__normal_3d_bottleneck
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_3d_conv_study__normal_3d_bottleneck
