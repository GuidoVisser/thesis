...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'Depth estimation is added to the models input, however the depth reconstruction counts for less in the total loss.' \
            --use_depth  \
...
mkdir -p $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__depth_lower_lambda
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__depth_lower_lambda

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO depth_study__depth_lower_lambda $t >> $HOME/thesis/times.txt