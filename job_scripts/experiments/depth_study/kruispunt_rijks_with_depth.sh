...
mkdir -p $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__with_depth
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__with_depth

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO depth_study__with_depth $t >> $HOME/thesis/times.txt