...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'Depth estimation is added to the models input, however the depth reconstruction loss is added to the total loss after a short rollin period' \
            --use_depth  \
            --lambda_recon_depth 0.0, 250 1.0

echo "$SLURM_JOBID | End:   $(date)" >> $HOME/thesis/job_logs/run_layer_decomposition.log

#Copy output directory from scratch to home
mkdir -p $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__depth_rollin_early
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/$VIDEO__$SLURM_JOBID__depth_study__depth_rollin_early

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO depth_study__depth_rollin_early $t >> $HOME/thesis/times.txt