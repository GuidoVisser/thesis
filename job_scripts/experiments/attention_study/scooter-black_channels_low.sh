...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'low number of attention channels' \
            --keydim 8 \
            --valdim 16
...
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_attention_study__channels_low
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_attention_study__channels_low

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO attention_study__channels_low $t >> $HOME/thesis/times.txt