...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'ground truth rgb input to the memory encoder, no shared backbone' \
            --memory_setup 2 \
            --model_setup 3
...
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_attention_study__rgb
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_attention_study__rgb

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO attention_study__rgb $t >> $HOME/thesis/times.txt