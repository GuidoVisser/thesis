...
python $HOME/thesis/run_layer_decomposition.py \
            ...
            --valdim 128 \
            --timesteps 4 \
            --description 'l1 and l0 on alpha composite with corr+diff dynamics regularization' \
            --corr_diff 

echo "$SLURM_JOBID | End:   $(date)" >> $HOME/thesis/job_logs/run_layer_decomposition.log

#Copy output directory from scratch to home
mkdir -p $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_alpha_study__alpha_composite_corr_diff
cp -RT $TMPDIR/output_dir $HOME/thesis/results/layer_decomposition/${VIDEO}_${SLURM_JOBID}_alpha_study__alpha_composite_corr_diff

read -r t<$TMPDIR/output_dir/time.txt
echo $SLURM_JOBID $VIDEO alpha_study__alpha_composite_corr_diff $t >> $HOME/thesis/times.txt