...
#         spatiotemporal_noise    = self.background_volume.spatiotemporal_noise.permute(1, 0, 2, 3) #    [Nf, C-3, H, W]
#         spatiotemporal_noise_t0 = F.grid_sample(spatiotemporal_noise[idx:idx+1],   background_uv_map_t0.unsqueeze(0)) #    [1, C-3, H, w]
#         spatiotemporal_noise_t1 = F.grid_sample(spatiotemporal_noise[idx+1:idx+2], background_uv_map_t1.unsqueeze(0)) #    [1, C-3, H, w]
...
class InputProcessor(object):
    ...
    def __getitem__(self, idx):
        ...
        # Add background layers to masks
        masks = torch.cat((torch.zeros_like(masks[0:1]).repeat(2, 1, 1, 1, 1), masks))
