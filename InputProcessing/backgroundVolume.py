...
class BackgroundVolume(object):
    ...
    def __init__(self,
        ...
        if path.exists(path.join(save_dir, "spatiotemporal_noise.pth")):
            ...
            self.spatial_noise = torch.randn(in_channels - 3 - int(use_depth), frame_size[1] // 16, frame_size[0] // 16)
            ...
            if num_static_channels < in_channels: # always true except with Omnimatte
                spatiotemporal_noise = torch.randn(in_channels - num_static_channels - 3 - int(use_depth), ceil(num_frames/temporal_coarseness), frame_size[1] // 16, frame_size[0] // 16)
