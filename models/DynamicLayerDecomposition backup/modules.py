...
class LayerDecompositionUNet(nn.Module):
    def __init__(self, memory_reader, conv_channels=64, in_channels=16, max_frames=200, coarseness=10, do_adjustment=True):
        ...
        self.valdim = memory_reader.valdim

        self.register_buffer("contexts", torch.zeros((memory_reader.valdim, memory_reader.keydim)))

        decoder_in_channels = conv_channels * 3 * 4 + self.valdim * 2
        ...
        self.decoder = nn.ModuleList([
            ...
            ConvBlock(nn.ConvTranspose2d, conv_channels * 3 * 4, conv_channels * 2, ksize=4, stride=2, norm=nn.BatchNorm2d),  # 1/4
            ConvBlock(nn.ConvTranspose2d, conv_channels * 3 * 2, conv_channels,     ksize=4, stride=2, norm=nn.BatchNorm2d),  # 1/2
            ConvBlock(nn.ConvTranspose2d, conv_channels * 3,     conv_channels,     ksize=4, stride=2, norm=nn.BatchNorm2d)]) # 1
    ...
    def render(self, x: torch.Tensor, context: torch.Tensor, mem_skips: Union[list, None]):
        ...
        for layer in self.decoder:
            ...
            if mem_skips == None:
                mem_skip = torch.zeros_like(skip)
            else:
                mem_skip = mem_skips.pop()

            x = torch.cat((x, skip, mem_skip), 1)
            x = layer(x)
    ...
    def forward(self, input, disp=False):
        ...
        index             = input["index"]
        attention_query   = input["attention_query"]
        ...
        attention_query = torch.cat((attention_query[:, 0], attention_query[:, 1]))
        ...
        for i in range(N_layers):
            ...
            if self.use_context:
                context_volume, mem_skips = self.memory_reader(attention_query, self.contexts)
                context_volumes.append(context_volume)
            else:
                context_volume = torch.zeros(batch_size * 2, self.valdim * 2, H//16, W//16).to(layer_input.device)
                mem_skips = None

            rgba, flow = self.render(layer_input, context_volume, mem_skips)
            ...
            # Object layers
                ...
                # if i == 1 and not self.use_context:
                #     rgba = -1 * torch.ones_like(rgba).to(rgba.device)
                #     alpha = torch.zeros_like(alpha).to(alpha.device)
