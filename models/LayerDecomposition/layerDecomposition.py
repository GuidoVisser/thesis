...
class LayerDecompositer(nn.Module):
    ...
    def visualize_and_save_output(self, model_output, i, epoch):
        ...
        background_rgb = rgba_layers[0, 0, 0, :3]
        foreground_rgb = rgba_layers[0, 0, 1, :3]

        foreground_alpha = rgba_layers[0, 0, 1, 3]

        background_img = background_rgb.permute(1, 2, 0).detach().cpu().numpy()
        foreground_img = foreground_rgb.permute(1, 2, 0).detach().cpu().numpy()
        alpha_img      = foreground_alpha.detach().cpu().numpy()
