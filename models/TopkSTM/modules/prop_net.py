...
class Decoder(nn.Module):
    def __init__(self, out_channels=1):
        ...
        self.pred = nn.Conv2d(256, out_channels, kernel_size=(3,3), padding=(1,1), stride=1)
...
class PropagationNetwork(nn.Module):
    ...
    def get_query_values(self, frame):
        # print(f"prop_net.get_query_values() \n\tframe shape: {frame.size()} \n\tframe type : {type(frame)}\n")
    ...
    def segment_with_query(self, keys, values, f16, f8, f4, k16, v16):
        # print(f"prop_net.segment_with_query() \n\t keys type: {type(keys)} \n\tkeys size: {keys.size()} \n\tvalues type {type(values)} \n\tvalues size: {values.size()} \n\tf16 type {type(f16)} \n\tf16 size: {f16.size()} \n\tf8 type {type(f8)} \n\tf8 size: {f8.size()} \n\tf4 type {type(f4)} \n\tf4 size: {f4.size()} \n\tk16 type {type(k16)} \n\tk16 size: {k16.size()} \n\tv16 type {type(v16)} \n\tv16 size: {v16.size()}")
